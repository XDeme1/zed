...
use lsp::{
    ...
    SemanticTokenModifier, SemanticTokensServerCapabilities, ServerCapabilities,
};
...
impl LspCommand for SemanticTokensFull {
    type Response = lsp::SemanticTokens;
    ...
    async fn response_from_lsp(
        ...
        store: Model<LspStore>,
        _: Model<Buffer>,
        server_id: LanguageServerId,
        mut cx: AsyncAppContext,
    ) -> Result<lsp::SemanticTokens> {
        let Some(lsp::SemanticTokensResult::Tokens(semantic_tokens)) = message else {
            return Ok(lsp::SemanticTokens::default());
        };
        let _ = store.update(&mut cx, |store, _| {
            let Some(lsp) = store.language_server_for_id(server_id) else {
                return;
            };
            let capabilities = lsp.adapter_server_capabilities();
            let (token_types, _token_modifiers) = match capabilities
                .server_capabilities
            ...
            let server_state = store
                .as_local_mut()
                .and_then(|store| store.language_servers.get_mut(&server_id));
